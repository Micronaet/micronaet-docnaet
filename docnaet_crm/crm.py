...
class docnaet_document(osv.osv):
    ...
    def schedule_import_docnaet_deadlined(self, cr, uid, context=None):
        """ Action called by the schedule for import deadlined documents
            I there's some document deadlined today mail are sent to users
        """
        # --------------------
        # Docnaet Documents
        # --------------------
        ...
        _logger.info(
            "Scheduled function: import_document_deadlined > "
            "Start import deadlined document!")

        # Load all document with deadline set up
        # TODO write function: import_document_deadlined
        doc_deadlined = self.import_document_deadlined(
            cr, uid, context=context)
        if doc_deadlined:
            _logger.info(
                "Scheduled function: import_document_deadlined > "
                "Document deadlined imported!")
        else:
            _logger.error(
                "Scheduled function: import_document_deadlined > "
                "Can't import deadlined document from Docnaet!")
        ...
        update_meeting_ids = calendar_pool.search(cr, uid, [
            ('docnaet_document_id', 'not in', doc_deadlined),
            ('docnaet_alert', '=', True)], context=context)
        calendar_pool.write(cr, uid, update_meeting_ids, {
            'docnaet_alert': False,
        }, context=context)  # remove flag docnaet alarm

        # Update calendar deadline for new elements
        update_meeting_ids = calendar_pool.search(cr, uid, [
            ('docnaet_document_id', 'in', doc_deadlined),
        ], context=context)
        to_create = doc_deadlined[:]

        for event in calendar_pool.browse(
                cr, uid, update_meeting_ids, context=context):
            # Update only description and deadline (if yet present)
            data = {'name': event.docnaet_document_id.name,
                    'description':
                        "Oggetto:\n%s\nDescrizione:\n%s\nNote:\n%s" % (
                            event.docnaet_document_id.name,
            ...
            if event.docnaet_document_id.deadline:  # deadline present
                data['date'] = "%s 07:00:00" % \
                               event.docnaet_document_id.deadline
                data['date_deadline'] = "%s 07:15:00" % \
                                        event.docnaet_document_id.deadline
        ...
        # -------------------------------
        alert_ids = calendar_pool.search(cr, uid, [
            ('docnaet_alert', '=', True),
        ], context=context)  # TODO deadline test ('date','<=',today)!!!!!!!!

        # Mail the events that are deadlined today
        link_for_user = {}
        for event in calendar_pool.browse(cr, uid, alert_ids, context=context):
            try:
                if event.user_id.id not in link_for_user:
                    link_for_user[event.user_id.id] = ""

                if event.docnaet_document_id:
                    document_id = event.docnaet_document_id.id
                    link = self.call_url(
                        cr, uid, document_id, docnaet_format=False,
                        context=context)['url']
                    link1 = "<a href='%s'>Docnaet</a>" % link
                    link = self.call_url(
                        cr, uid, document_id, docnaet_format=True,
                        context=context)['url']
            ...
            link_for_user[event.user_id.id] += \
                "<strong>Prot. %s</strong>: %s [%s - %s] <br/>\n" % (
                    event.docnaet_document_id.protocol_id.name if (
                            event.docnaet_document_id and
                            event.docnaet_document_id.protocol_id)
                    else "Nessun protocollo",
                    event.name,
                    link1,
                    link2,
                )
        ...
        compose_message_pool = self.pool.get('mail.compose.message')
        # Message (for render template)

        template_ids = template_pool.search(cr, uid, [
            ('name', '=', 'Alert CRM meeting'),
            ('model', '=', 'res.users'),
        ], context=context)
        if not template_ids:
            _logger.error("Template not found, mail not sent!")

        template_proxy = template_pool.browse(
            cr, uid, template_ids, context=context)[0]
        mail_ids = []
        for user in link_for_user.keys():  # loop on all users ID
            email_subject = compose_message_pool.render_template(
                cr, uid, _(template_proxy.subject), 'res.users', user)
            body_html = compose_message_pool.render_template(
                cr, uid, _(template_proxy.body_html), 'res.users', user)
            body_html = body_html.replace(
                "<!--micronaet-->", link_for_user[user])
            email_to = compose_message_pool.render_template(
                cr, uid, _(template_proxy.email_to), 'res.users', user)
            reply_to = compose_message_pool.render_template(
                cr, uid, _(template_proxy.reply_to), 'res.users', user)
            email_from = compose_message_pool.render_template(
                cr, uid, _(template_proxy.email_from), 'res.users', user)
            ...
            mail_ids.append(mail_pool.create(cr, uid, {
                ...
                'mail_server_id':
                    template_proxy.mail_server_id and
                    template_proxy.mail_server_id.id or False,
                'state': 'outgoing',
        ...
        _logger.info(
            "Scheduled function: "
            "End import deadlined and alert mail operation!")
        return True
...
class crm_calendar(osv.osv):
    ...
    def button_remove_docnaet_alert(self, cr, uid, ids, context=None):
        ...
        if event.docnaet_document_id:
            query = "UPDATE Documenti SET docScadenza = '' " \
                    "WHERE ID_documento = %s" % \
                    event.docnaet_document_id.docnaet_document_id,
            print("QUERY: %s" % query)
            if not self.pool.get('docnaet.document').docnaet_run_command_sql(
